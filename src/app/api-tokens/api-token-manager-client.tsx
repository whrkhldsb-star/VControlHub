...
<StatCard label="Token 总数" value={tokens.length} />
      ...
      {createdPlaintext && (
        <section className="rounded-2xl border border-amber-300/25 bg-amber-300/[0.08] p-5 shadow-[0_20px_80px_rgba(251,191,36,0.08)]">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <h2 className="text-base font-semibold text-amber-100 light:text-amber-900">一次性明文 Token</h2>
              <p className="mt-1 text-sm text-amber-100/75 light:text-amber-900/75">请立即复制，此明文 Token 离开页面后无法再次查看。</p>
            </div>
            <button type="button" onClick={() => navigator.clipboard?.writeText(createdPlaintext)} className="rounded-xl border border-amber-200/25 px-3 py-2 text-xs font-medium text-amber-100 light:text-amber-900 hover:bg-amber-200/10">
              复制 Token
            </button>
          </div>
          <code className="mt-4 block overflow-x-auto rounded-xl border border-amber-200/20 bg-slate-950/70 light:bg-white/70 p-3 font-mono text-xs text-amber-100 light:text-amber-900">{createdPlaintext}</code>
        </section>
      )}

      {error && <div className="rounded-xl border border-rose-400/25 bg-rose-400/[0.08] px-4 py-3 text-sm text-rose-100 light:text-rose-900">{error}</div>}

      <section className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5">
        <h2 className="text-lg font-semibold text-white light:text-slate-900">创建 API Token</h2>
        ...
        <form onSubmit={createToken} className="mt-5 grid gap-4">
          <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
            <label className="space-y-1.5">
              <span className="text-xs font-medium tracking-wide text-white light:text-slate-900/50">Token 名称</span>
              <input value={name} onChange={(event) => setName(event.target.value)} required maxLength={80} placeholder="例如：移动端 CLI / 监控脚本" className="w-full rounded-xl border border-white/[0.06] bg-white/[0.04] px-3.5 py-2.5 text-sm text-white light:text-slate-900 outline-none placeholder:text-white/20 focus:border-cyan-400/30" />
            </label>
            <label className="space-y-1.5">
              <span className="text-xs font-medium tracking-wide text-white light:text-slate-900/50">过期时间（可选）</span>
              <input type="datetime-local" value={expiresAt} onChange={(event) => setExpiresAt(event.target.value)} className="w-full rounded-xl border border-white/[0.06] bg-white/[0.04] px-3.5 py-2.5 text-sm text-white light:text-slate-900 outline-none focus:border-cyan-400/30" />
            </label>
          </div>

          <div className="space-y-2">
            <div className="text-xs font-medium tracking-wide text-white light:text-slate-900/50">权限范围</div>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-white light:text-slate-900">Token 列表</h2>
                  ...
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <h3 className="text-base font-semibold text-white light:text-slate-900">{token.name}</h3>
                        <span className={`rounded-full border px-2 py-0.5 text-[11px] ${status.className}`}>{status.label}</span>
                      </div>
                      <p className="mt-2 font-mono text-xs text-slate-400 light:text-slate-600">{token.tokenPrefix}…{token.tokenSuffix}</p>
                      <div className="mt-3 flex flex-wrap gap-1.5">
                        {token.scopes.map((scope) => <span key={scope} className="rounded-md border border-white/[0.06] bg-white/[0.04] px-2 py-0.5 font-mono text-[11px] text-slate-400 light:text-slate-600">{scope}</span>)}
                      </div>
                    ...
                    {!token.revokedAt && (
                      <button type="button" aria-label={`撤销 ${token.name}`} disabled={revokingId === token.id} onClick={() => setTokenPendingRevoke(token)} className="rounded-2xl border border-rose-400/30 bg-rose-400/10 px-4 py-2 text-xs font-medium text-rose-100 light:text-rose-900 transition hover:bg-rose-400/20 disabled:opacity-60">
      ...
      {tokenPendingRevoke && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 light:bg-white/70 px-4 backdrop-blur-sm" role="presentation">
          <section role="dialog" aria-modal="true" aria-labelledby="revoke-api-token-title" className="w-full max-w-md rounded-2xl border border-rose-400/25 bg-slate-950 light:bg-white p-6 shadow-[0_24px_100px_rgba(244,63,94,0.16)]">
            <h2 id="revoke-api-token-title" className="text-lg font-semibold text-white light:text-slate-900">确认撤销 API Token</h2>
            <p className="mt-3 text-sm leading-6 text-slate-300 light:text-slate-700">
              即将撤销 Token <span className="font-semibold text-rose-100 light:text-rose-900">{tokenPendingRevoke.name}</span>。撤销后脚本、CLI 或外部集成将立即失去访问权限，且无法恢复。
            </p>
            <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              <button type="button" onClick={() => setTokenPendingRevoke(null)} className="rounded-xl border border-white/[0.08] px-4 py-2 text-sm font-medium text-slate-200 light:text-slate-800 hover:bg-white/[0.06]">
                取消
              </button>
              <button type="button" onClick={() => revokeToken(tokenPendingRevoke)} className="rounded-xl bg-rose-500 px-4 py-2 text-sm font-semibold text-white light:text-slate-900 hover:bg-rose-400">
                确认撤销
              </button>
            </div>
          </section>
