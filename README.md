...
- [ ] **图床删除授权过宽（P0）。** 当前 `/api/images/[id]` 删除接口把 `user:read` 当成跨用户删除许可，而默认 viewer 角色也包含 `user:read`；这会导致普通可登录用户在知道图片 ID 时删除他人图片。需要改为所有者或明确的 `image:manage`/`storage:delete` 类权限。
- [ ] **本机文本文件编辑绕过细粒度存储授权（P0）。** `/api/files/editable/[id]` 只检查全局 `storage:read/write`，没有复用存储节点/路径授权；拥有全局存储权限但没有具体节点授权的用户，可能按文件条目 ID 读取或覆盖本机可编辑文本文件。
...
- [ ] **既有增强项仍在队列中。** 快捷服务还缺配置 diff、失败回滚和历史更新日志；备份策略还缺异地备份、自动恢复演练和保留策略自动清理；本机文本编辑还缺并发修改提示、保存后可选重载服务和 SFTP 编辑；媒体库/图床还可补图片目录批量选择、发布历史和更完整的相册/标签管理。
