		...
		<form action={formAction} data-card className=" p-5 space-y-4">
			...
			<div className="grid gap-3 md:grid-cols-2">
				<label className="grid gap-1.5 text-xs font-medium text-[var(--text-secondary)]">
					标题
					<input name="subject" required placeholder="简要描述问题" className="rounded-lg border border-white/[0.08] bg-slate-950 px-3 py-2 text-sm text-slate-100 placeholder:text-slate-600 light:placeholder:text-slate-500" />
				</label>
				<label className="grid gap-1.5 text-xs font-medium text-[var(--text-secondary)]">
					优先级
			...
			<label className="grid gap-1.5 text-xs font-medium text-[var(--text-secondary)]">
				描述
				<textarea name="description" required rows={4} placeholder="详细描述你的需求或遇到的问题" className="rounded-lg border border-white/[0.08] bg-slate-950 px-3 py-2 text-sm text-slate-100 placeholder:text-slate-600 light:placeholder:text-slate-500 resize-y" />
			</label>
