...
| 2 | **大文件拆分继续推进 — 当前生产非测试非字典仍有 11 个 >500 行源文件**；上一轮把 `cost-page-client.tsx` 668→481、`image-bed-page-client.tsx` 653→496、`downloads-client.tsx` 645→356、`quick-services-client.tsx` 598→491、`server-overview-details.tsx` 592→379；本轮继续把 `files/preview/text-preview-client.tsx` 629→498，并提取 `text-preview-renderers.tsx` / `text-preview-states.tsx` / `text-preview-highlight.ts`。当前 `src/app` 真实页面组件 >500 已清零（测试文件不计入生产拆分目标） | 已处理：`ai-ops-page-client.tsx`、`cost-summary/*`、`image-bed/*`、`downloads/*`、`quick-services/*`、`servers/server-overview-*`、`files/preview/text-preview-*`；当前剩余生产热点主要转为非页面/底层模块：`ssh-file-manager.tsx`(602)、`quick-service/service-lifecycle.ts`(636)、`quick-service/catalog.ts`(601)、`server/service-profiles.ts`(554)、`ai/chat/route.ts`(543) 等 | 继续拆分：优先 SSH 文件管理、quick-service lifecycle/catalog 与服务端 profile/import 逻辑 |
