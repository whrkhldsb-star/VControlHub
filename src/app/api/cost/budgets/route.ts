...
export async function GET(request: Request) {
	return withApiRoute(request, { permission: "cost:read", rateLimit: GENERAL_WRITE_LIMIT, errorMessage: "Failed to load cost budgets" }, async () => {
		return NextResponse.json({ budgets: await listCostBudgets() });
	});
}
...
export async function POST(request: Request) {
	return withApiRoute(request, { permission: "cost:manage", rateLimit: GENERAL_WRITE_LIMIT, bodySchema: createCostBudgetSchema, errorStatus: 400, errorMessage: "Failed to create cost budget" }, async ({ session, body }) => {
		const budget = await createCostBudget(body);
