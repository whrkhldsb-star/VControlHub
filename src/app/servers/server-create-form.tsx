...
export function ServerCreateForm({
 ...
 data-card className="grid gap-4 "
    >
      ...
      {state.error && (
        <div className="rounded-lg bg-rose-500/[0.10] border border-rose-400/20 px-3.5 py-2.5 text-sm text-rose-200">
          {state.error}
        </div>
      )}
      {state.success && (
        <div className="rounded-lg bg-emerald-500/[0.10] border border-emerald-400/20 px-3.5 py-2.5 text-sm text-emerald-200">
          {state.success}
        </div>
      )}
      ...
      <label data-tone="cyan" className="rounded-xl border border-[var(--color-action-border)]/20 p-4 text-sm text-[var(--text-secondary)]">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <label className="mt-3 block text-xs font-medium text-[var(--text-primary)]/70" htmlFor="directGatewayProtocol">
