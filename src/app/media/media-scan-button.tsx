...
export function MediaScanButton() {
	...
	async function handleScan() {
		...
		setError(null);
		try {
			const result = await csrfFetch<ScanResult>("/api/media", { method: "POST" });
			setMessage(`扫描完成：发现 ${result.scanned} 个媒体文件，更新 ${result.upserted} 条索引`);
			startTransition(() => router.refresh());
		} catch (scanError) {
			setError(scanError instanceof Error ? scanError.message : "扫描媒体索引失败");
		} finally {
			setIsScanning(false);
		}
	}
