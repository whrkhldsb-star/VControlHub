...
- **设置页高风险设置（TR-014, P2）** — 当前值/配置来源/生效位置/重启边界、最近修改人+时间已展示；M01 完成：字段级 ↺ 恢复默认按钮（高/中风险 badge）+ Save 角标 "X 项已修改"（含 diff 表）+ high 风险提交时弹确认 modal；M02 完成：高风险字段失焦且已变时 inline 警告条 ⚠ 提示保存前请确认影响（rose 色 + `role="alert"` 同步屏幕阅读器 + `aria-describedby` 关联 + 继续改值/保存后自动清除）。
