...
export function CommandCreateForm({
	...
	return (
		<form action={formAction} className="grid gap-4 rounded-xl border border-white/[0.06] bg-white/[0.02] p-5">
			<div>
				<h2 className="text-lg font-semibold text-white light:text-slate-900">下发命令</h2>
			...
			<div className="space-y-1.5">
				<label className="text-xs font-medium text-white light:text-slate-900/50 tracking-wide" htmlFor="cmdTitle">命令标题</label>
				<input id="cmdTitle" name="title" type="text" required placeholder="例如 检查磁盘空间" className="w-full rounded-lg border border-white/[0.06] bg-white/[0.04] px-3.5 py-2.5 text-sm text-white light:text-slate-900 outline-none transition placeholder:text-white/20 focus:border-cyan-400/30 focus:bg-white/[0.06]" />
			</div>

			<div className="space-y-1.5">
				<label className="text-xs font-medium text-white light:text-slate-900/50 tracking-wide" htmlFor="cmdCommand">命令内容</label>
			...
			<div className="space-y-1.5">
				<label className="text-xs font-medium text-white light:text-slate-900/50 tracking-wide" htmlFor="cmdReason">原因 / 备注</label>
				<textarea id="cmdReason" name="reason" rows={2} placeholder="可选" className="w-full rounded-lg border border-white/[0.06] bg-white/[0.04] px-3.5 py-2.5 text-sm text-white light:text-slate-900 outline-none transition placeholder:text-white/20 focus:border-cyan-400/30 focus:bg-white/[0.06] resize-y" />
			</div>
								...
								className={`flex items-center gap-2.5 rounded-lg border px-3 py-2 text-sm cursor-pointer transition ${
									!server.enabled
										...
										: selectedServerIds.has(server.id)
											?"border-cyan-400/20 bg-cyan-400/[0.06] text-white light:text-slate-900"
