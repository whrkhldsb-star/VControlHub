...
| 2 | **大文件拆分继续推进 — 当前生产非测试非字典仍有 8 个 >500 行源文件**；上一轮把 `cost-page-client.tsx` 668→481、`image-bed-page-client.tsx` 653→496、`downloads-client.tsx` 645→356、`quick-services-client.tsx` 598→491、`server-overview-details.tsx` 592→379；后续把 `files/preview/text-preview-client.tsx` 629→498，并提取 `text-preview-renderers.tsx` / `text-preview-states.tsx` / `text-preview-highlight.ts`；再把 `ssh-file-manager.tsx` 602→238，并提取 `ssh-file-manager-parts.tsx` / `ssh-file-manager-list.tsx` / `ssh-file-manager-dialogs.tsx`；本轮继续把 `ssh-terminal-modal.tsx` 530→461，并提取 `ssh-terminal-side-panel.tsx`。当前 `src/app` 真实页面组件 >500 已清零，`src/components` 生产组件 >500 已清零（测试文件不计入生产拆分目标） | 已处理：`ai-ops-page-client.tsx`、`cost-summary/*`、`image-bed/*`、`downloads/*`、`quick-services/*`、`servers/server-overview-*`、`files/preview/text-preview-*`、`ssh-file-manager*`、`ssh-terminal-*`；当前剩余生产热点主要转为非页面/底层模块：`quick-service/service-lifecycle.ts`(636)、`quick-service/catalog.ts`(601)、`server/service-profiles.ts`(554)、`ai/chat/route.ts`(543) 等 | 继续拆分：优先 quick-service lifecycle/catalog、server profile/import 与 API route 逻辑 |
| 3 | **硬编码中文字符串继续收敛中** — 本轮聚焦架构拆分且未新增可见中文硬编码；最新精扫区分测试/API/注释后，`src/app` + `src/components` 真实前端面仍约 29 文件 / 108 行，主要集中在 server action 返回消息、文件操作 toast、Provider 外 `global-error`、VPS backup 双语 preset、nav fallback 和少量封装/压缩行 | 已迁移：`dashboard-localized-sections.tsx`、`settings/system-config-section.tsx`、`image-bed-page-client.tsx`、`files/*` 多个操作组件、`backups/*` 操作按钮、`operation-tasks/page.tsx`、`scheduled-tasks/page.tsx`、`quick-services/page.tsx`、`ai-ops-page-client.tsx`；本轮拆分未引入缺失 i18n key，`i18n:key-check` 通过 | 下一步按可见度处理：`files/file-entry-utils.ts`、`files/use-file-batch-operations.ts`、`global-error.tsx`、`servers/actions.ts`，低风险 fallback/双语 label 单独标注例外 |
