  ...
  const file: FileProp = {
    ...
    directAccessHref:
      isDirectAccess && node.publicBaseUrl
        ? `${node.publicBaseUrl.replace(/\/$/, "")}/${item.relativePath.split("/").map(encodeURIComponent).join("/")}`
        : null,
...
function MediaCover({
  ...
  const className =
    "relative block aspect-[4/3] overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--surface-subtle)]";
...
export function MediaItemCard({
  ...
  const publishAsImageBed = async () => {
    ...
    try {
      ...
      setPublishError(
        getErrorMessage(error, t("mediaItemCard.publishErrorFallback")),
      );
    } finally {
      setPublishing(false);
    }
