    ...
    <div data-card className="group p-4 transition hover:bg-[var(--surface-elevated)]">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="rounded-lg border border-[var(--border)] bg-[var(--surface-elevated)] px-2 py-0.5 text-xs text-[var(--text-muted)]">{s.language}</span>
          {s.isPrivate && <span className="text-xs text-[var(--warning)]">{t("snippetsPage.private")}</span>}
          {s.tags.length > 0 && (
            <div className="flex flex-wrap gap-1">
              {s.tags.map((tag) => (
                <span key={tag} data-tone="cyan" className="rounded-lg border border-[var(--accent-border)] px-2 py-0.5 text-xs text-[var(--accent)]">{tag}</span>
              ))}
            </div>
          )}
        </div>
