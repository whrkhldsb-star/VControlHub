...
| 3 | **大量 `.tsx` 组件仍含硬编码中文字符串** — 绕过 `t()` i18n 系统，EN locale 下直接显示中文；当前生产非注释粗扫约 33 文件 / 130 行（宽扫含注释约 63 文件 / 228 行），`audit/page.tsx`、`files/file-batch-toolbar.tsx`、`requests/page.tsx`、`requests/batch-review-toolbar.tsx`、`status/page.tsx`、`files/*` 多个操作组件、`backups/*` 操作按钮、`server-overview-card.tsx` 已迁移到字典 | 当前热点：`dashboard-localized-sections.tsx`、`settings/system-config-section.tsx`、`image-bed-page-client.tsx`、`vps-backup-section.tsx`、`storage-node-delete-button.tsx`、`deployments/page.tsx` 等 | 将所有 JSX 文本/错误消息迁移到 `t()` + 字典 key；`i18n:key-check` 脚本仅校验已用 key 的完整性，无法检测绕过 `t()` 的硬编码中文 |
