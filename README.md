...
- [ ] **前端可访问性、移动端和浏览器导航仍需系统化收口（P2）。** 全局搜索已具备 `dialog` 语义、初始聚焦、Escape 关闭和 opener focus restore；Docker 日志弹窗已接入统一 Dialog Focus hook；文件浏览 SPA 已支持目录导航 pushState 与 popstate 恢复；SSH 终端仍缺 Escape/focus trap/focus restore，且固定 `minHeight: 400px`、横向命令侧栏在手机上仍易拥挤；文本预览搜索/跳转、SSH 常用命令输入等仍依赖 placeholder 而缺显式 label。
