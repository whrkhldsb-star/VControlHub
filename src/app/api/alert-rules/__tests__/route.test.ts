...
vi.mock("@/lib/health/service", () => ({ evaluateAlerts: mocks.evaluateAlerts }));
...
describe("/api/alert-rules", () => {
  beforeEach(() => {
    ...
    mocks.listAlertRules.mockResolvedValue([{ id: "rule1", name: "CPU", webhookUrl: "https://hooks.example.com/secret" }]);
    mocks.createAlertRule.mockResolvedValue({ id: "rule1", name: "CPU", webhookUrl: "https://hooks.example.com/secret" });
  ...
  it("uses notification:manage rather than user:manage for alert rule access", async () => {
    await route.GET();
    expect(mocks.sessionHasPermission).toHaveBeenCalledWith(expect.anything(), "notification:manage");
    expect(mocks.sessionHasPermission).not.toHaveBeenCalledWith(expect.anything(), "user:manage");
  });
  ...
  it("rejects invalid alert rule payloads before persistence", async () => {
    const req = new Request("http://local/api/alert-rules", {
      ...
      body: JSON.stringify({ name: "", metric: "load_avg", operator: "gte", threshold: 1000, notifyChannels: ["webhook"], webhookUrl: "ftp://internal" }),
    });
  ...
  it("creates alert rules with validated fields and audits without webhook secrets", async () => {
    const req = new Request("http://local/api/alert-rules", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ name: " CPU hot ", metric: "cpu_usage", operator: "gte", threshold: 90, notifyChannels: ["in_app", "webhook"], webhookUrl: "https://hooks.example.com/secret", cooldownMinutes: 10, serverIds: ["srv1"] }),
    });
    const res = await route.POST(req);
    expect(res.status).toBe(201);
    expect(mocks.createAlertRule).toHaveBeenCalledWith(expect.objectContaining({ name: "CPU hot", metric: "cpu_usage", operator: "gte", threshold: 90, notifyChannels: ["in_app", "webhook"], webhookUrl: "https://hooks.example.com/secret", cooldownMinutes: 10, serverIds: ["srv1"] }));
    expect(mocks.auditUserAction).toHaveBeenCalledWith("u1", "alert_rule.create", expect.objectContaining({ ruleId: "rule1", webhookConfigured: true }));
  ...
  it("accepts form submissions and redirects back to alert rules page", async () => {
    const req = new Request("http://local/api/alert-rules", {
      ...
      body: new URLSearchParams([["name", "CPU"], ["metric", "cpu_usage"], ["operator", "gte"], ["threshold", "90"], ["notifyChannels", "in_app"]]),
    });
    const res = await route.POST(req);
  ...
  it("rejects updates that enable webhook notifications without a webhook URL", async () => {
    const res = await route.PATCH(new Request("http://local/api/alert-rules", {
      method: "PATCH",
      headers: { "content-type": "application/json" },
  ...
  it("rejects webhook URLs pointing at local or private network targets", async () => {
    for (const webhookUrl of ["https://localhost/hook", "https://127.0.0.1/hook", "https://10.0.0.5/hook", "https://192.168.1.2/hook", "https://[::1]/hook"]) {
      const res = await route.POST(new Request("http://local/api/alert-rules", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ name: "Private hook", metric: "cpu_usage", operator: "gte", threshold: 90, notifyChannels: ["webhook"], webhookUrl }),
      }));
      expect(res.status).toBe(400);
    }
    expect(mocks.createAlertRule).not.toHaveBeenCalled();
