...
- **Q 兼容层补齐 + 冗余 `light:` 修饰符批量清理（4 轮 ~505 处）** —
  - **R1 Q5b 补齐** — `globals.css` 新增 `html.light .border-slate-200/300/400 { border-color: var(--border) }` 段（1158-1166），与既有 `border-slate-700~950` 路径对齐。
  - **R2 批量删 4 大类（−431 处）** — `light:bg-white` / `light:border-slate-200/300/400` / `light:text-slate-500..950` / `light:bg-slate-50/100/200/300`（无透明度变体）。Q 兼容层 R1–R7 + Q1–Q7 段已 100% 覆盖这些类在 light 模式到 `--surface` / `--border` / `--text-primary` 的映射，源码双写为纯冗余。71 文件 +322/−322 完全对称 = 纯删除。
  - **R3 `light:bg-slate-{50,100,200,300}` 无透明度补删（−58 处）** — 与 R2 同理，Q 层 L281-307 已接管 `bg-slate-700/50` `bg-slate-900/30` 等深色 slate+透明度变体。32 文件 +58/−58。
  - **R4 `light:bg-slate-900/{20,30,50,60}` 模态遮罩（−16 处）** — Q 层 L1501-1510 段已覆盖 `fixed inset-0.bg-black/60` 及子透明度变体在 light 模式映射，模态遮罩在 light 模式自动由 Q 层接管。9 文件 +16/−16。
