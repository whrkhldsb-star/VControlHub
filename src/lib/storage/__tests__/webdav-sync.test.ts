...
beforeEach(() => {
  ...
  mocks.transaction.mockImplementation(async (fn) => fn({ fileEntry: mocks.fileEntry, $queryRaw: mocks.queryRaw }));
});
...
describe("WebDAV inventory sync", () => {
  ...
  it("counts unchanged entries as synced without writing or changing updatedAt", async () => {
    ...
    mocks.queryRaw.mockResolvedValueOnce([unchanged("same", "same.txt")]);
  ...
  it("does not re-read or update a fully inserted batch", async () => {
    ...
    expect(mocks.fileEntry.findMany).not.toHaveBeenCalled();
    expect(mocks.fileEntry.updateMany).not.toHaveBeenCalled();
  });
  ...
  it("does not rewrite inserted metadata when resolving partial batch conflicts", async () => {
    ...
    mocks.fileEntry.findMany.mockResolvedValueOnce([unchanged("new", "new.txt"), { ...unchanged("raced", "raced.txt"), size: BigInt(1) }]);
  ...
  it("reports retained missing indexes instead of silently claiming a clean inventory", async () => {
    mocks.queryRaw.mockResolvedValueOnce([row("protected", "private-secret.txt")]);
  ...
  it("reads existing entries in batches and never resurrects tombstones", async () => {
    ...
    mocks.queryRaw.mockResolvedValueOnce([row("live", "live.txt"), row("trash", "trash.txt", true)]);
    const result = await syncWebDavDirectoryEntries({ node });
  ...
  it("removes only missing direct active indexes, allowing remote reappearance", async () => {
    mocks.queryRaw.mockResolvedValueOnce([row("gone", "dir/gone.txt"), row("trash", "dir/trash.txt", true)]);
  ...
  it("uses bounded create and conflict reads, preserving a concurrently created tombstone", async () => {
    mocks.list.mockResolvedValue([remote("new.txt"), remote("raced.txt")]);
    mocks.fileEntry.findMany.mockResolvedValueOnce([unchanged("new", "new.txt"), row("raced", "raced.txt", true)]);
    const result = await syncWebDavDirectoryEntries({ node });
  ...
  it("does not start pruning when an inventory write fails", async () => {
    ...
    mocks.queryRaw.mockResolvedValueOnce([row("live", "live.txt"), row("gone", "gone.txt")]);
  ...
  it("uses a serializable transaction and discards success counters when commit fails", async () => {
    mocks.queryRaw.mockResolvedValueOnce([row("gone", "gone.txt")]);
    mocks.transaction.mockImplementation(async (fn) => { await fn({ fileEntry: mocks.fileEntry, $queryRaw: mocks.queryRaw }); throw new Error("commit secret"); });
    const result = await syncWebDavDirectoryEntries({ node });
  ...
  it("paginates inventory before any pruning and aborts if a later page fails", async () => {
    mocks.queryRaw.mockResolvedValueOnce(Array.from({ length: 2000 }, (_, i) => row(`id-${i}`, `file-${i}`))).mockRejectedValueOnce(new Error("page failure"));
    const result = await syncWebDavDirectoryEntries({ node });
    expect(result.errors).toHaveLength(1);
    expect(mocks.fileEntry.deleteMany).not.toHaveBeenCalled();
    expect(mocks.fileEntry.updateMany).not.toHaveBeenCalled();
  ...
  it("prunes in bounded batches only after the complete paginated read", async () => {
    mocks.queryRaw.mockResolvedValueOnce(Array.from({ length: 2000 }, (_, i) => row(`id-${i}`, `file-${i}`))).mockResolvedValueOnce([row("last", "last.txt")]);
    mocks.fileEntry.deleteMany.mockImplementation(async ({ where }) => ({ count: where.id.in.length }));
    const result = await syncWebDavDirectoryEntries({ node });
    expect(result.deleted).toBe(2001);
    expect(mocks.queryRaw.mock.calls[1]![0].values).toContain("id-1999");
    ...
    expect(mocks.queryRaw.mock.invocationCallOrder[1]).toBeLessThan(mocks.fileEntry.deleteMany.mock.invocationCallOrder[0]!);
  });

  it("discards counters when a later prune batch fails", async () => {
    mocks.queryRaw.mockResolvedValueOnce(Array.from({ length: 501 }, (_, i) => row(`id-${i}`, `file-${i}`)));
  ...
  it("deduplicates remote entries and maps directory metadata", async () => {
    ...
    mocks.queryRaw.mockResolvedValueOnce([row("folder-id", "folder")]);
    const result = await syncWebDavDirectoryEntries({ node });
  ...
  it("keeps a large unchanged inventory write-free across read pages", async () => {
    ...
    mocks.queryRaw.mockResolvedValueOnce(rows.slice(0, 2000)).mockResolvedValueOnce(rows.slice(2000));
    const result = await syncWebDavDirectoryEntries({ node });
  ...
  it("rolls back reported inserts if a partial conflict cannot be resolved", async () => {
    ...
    mocks.fileEntry.findMany.mockResolvedValueOnce([unchanged("new", "new.txt")]);
    const result = await syncWebDavDirectoryEntries({ node });
    expect(result).toEqual({ synced: 0, created: 0, updated: 0, deleted: 0, errors: [expect.any(String)] });
    expect(mocks.fileEntry.deleteMany).not.toHaveBeenCalled();
  });
