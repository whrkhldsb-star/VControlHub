	...
	return (
		<div className="flex min-h-[40vh] flex-col items-center justify-center gap-4 p-8 text-center">
			...
			<div className="space-y-2">
				...
				<p className="max-w-lg text-sm leading-6 text-slate-400">{error.message || description}</p>
				{error.digest ? (
					<p className="rounded-full border border-white/10 bg-white/[0.03] px-3 py-1 text-xs text-slate-500">
						错误编号：{error.digest}
					</p>
				) : null}
			</div>
			<div className="flex flex-wrap justify-center gap-2">
				...
				<a
					href="/health"
					className="rounded-lg border border-white/10 bg-white/[0.03] px-4 py-2 text-sm font-medium text-slate-200 transition-colors hover:bg-white/[0.06]"
				>
					系统自检
				</a>
			</div>
