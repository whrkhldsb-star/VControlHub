...
| 3 | **硬编码中文字符串继续收敛中** — 本轮已迁移 AI 聊天附件/模型能力提示与 Files 预览/批量操作 toast 到 i18n 字典；最新精扫区分测试/API/注释后，`src/app` + `src/components` 真实前端面仍约 25 文件 / 116 行，主要集中在 server action 返回消息、Provider 外 `global-error`、VPS backup 双语 preset、nav fallback、文件移动 action 和少量封装/压缩行 | 已迁移：`dashboard-localized-sections.tsx`、`settings/system-config-section.tsx`、`image-bed-page-client.tsx`、`files/*` 多个操作组件、`backups/*` 操作按钮、`operation-tasks/page.tsx`、`scheduled-tasks/page.tsx`、`quick-services/page.tsx`、`ai-ops-page-client.tsx`、`ai/*` 附件提示、`files/file-entry-utils.ts`、`files/use-file-batch-operations.ts`；`i18n:key-check` 通过 | 下一步按可见度处理：`files/move-file-action.ts`、`files/use-file-browser-listing.ts`、`servers/actions.ts`、`requests/actions.ts`、`global-error.tsx`，低风险 fallback/双语 label 单独标注例外 |
