...
| 3 | **硬编码中文字符串继续收敛中** — 本轮已迁移 AI 聊天附件/模型能力提示、Files 预览/批量操作 toast、文件移动 action、文件列表刷新 fallback、审批中心与服务器管理 server actions、登录/改密/备份 action、global-error copy、nav fallback、通用权限提示到 i18n/英文 fallback；最新精扫区分测试/API/注释后，`src/app` + `src/components` 真实前端面仍约 13 文件 / 30 行，主要集中在压缩旧组件/注释误报、VPS backup 双语 preset、少量格式化替换桥接 | 已迁移：`dashboard-localized-sections.tsx`、`settings/system-config-section.tsx`、`image-bed-page-client.tsx`、`files/*` 多个操作组件、`backups/*` 操作按钮、`operation-tasks/page.tsx`、`scheduled-tasks/page.tsx`、`quick-services/page.tsx`、`ai-ops-page-client.tsx`、`ai/*` 附件提示、`files/file-entry-utils.ts`、`files/use-file-batch-operations.ts`、`files/move-file-action.ts`、`files/use-file-browser-listing.ts`、`requests/actions.ts`、`servers/actions.ts`、`servers/command-actions.ts`、`account/password/actions.ts`、`login/actions.ts`、`global-error.tsx`、`nav-items.tsx`；`i18n:key-check` 通过 | 下一步处理/标注例外：压缩旧组件扫描误报、`vps-backup-section.tsx` 双语 preset、`ui-primitives.tsx` 注释、`use-conversations.ts` 附件标题 sentinel、`storage-node-list.tsx` 标点 |
