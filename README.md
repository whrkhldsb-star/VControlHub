...
| 2 | **大文件拆分继续推进 — 当前生产非测试非字典仍有 12 个 >500 行源文件**；本轮已把 `cost-page-client.tsx` 668→481、`image-bed-page-client.tsx` 653→496、`downloads-client.tsx` 645→356、`quick-services-client.tsx` 598→491、`server-overview-details.tsx` 592→379，并新增按职责拆分的 shared/sections/dialog/row 组件；当前 `src/app` 真实页面组件 >500 已只剩 `files/preview/text-preview-client.tsx`（测试文件不计入生产拆分目标） | 已处理：`ai-ops-page-client.tsx`、`cost-summary/*`、`image-bed/*`、`downloads/*`、`quick-services/*`、`servers/server-overview-*`；当前剩余生产热点：`text-preview-client.tsx`(629)、`ssh-file-manager.tsx`(602)、`quick-service/service-lifecycle.ts`(636)、`quick-service/catalog.ts`(601)、`server/service-profiles.ts`(554)、`ai/chat/route.ts`(543) 等 | 继续拆分：优先 `text-preview-client.tsx` 编辑/预览渲染与保存 hook，然后处理 SSH 文件管理、quick-service lifecycle/catalog |
