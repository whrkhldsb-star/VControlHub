...
describe("/api/scheduled-tasks audit coverage", () => {
  ...
  it("audits scheduled task lifecycle changes without leaking command text", async () => {
    await route.POST(new Request("http://local/api/scheduled-tasks", {
      method: "POST",
    ...
    await route.PATCH(new Request("http://local/api/scheduled-tasks", {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ id: "task1", name: "Clean logs weekly", cron: "0 2 * * 1" }),
    }));
    ...
    await route.DELETE(new Request("http://local/api/scheduled-tasks?id=task1", { method: "DELETE" }));

    expect(mocks.auditUserAction).toHaveBeenCalledWith("u1", "scheduled_task.create", expect.objectContaining({ taskId: "task1", name: "Clean logs", cronExpression: "0 2 * * *", serverCount: 2, status: "ACTIVE" }));
    expect(mocks.auditUserAction).toHaveBeenCalledWith("u1", "scheduled_task.update", expect.objectContaining({ taskId: "task1", name: "Clean logs weekly", cronExpression: "0 2 * * 1", serverCount: 1, status: "ACTIVE" }));
    expect(mocks.auditUserAction).toHaveBeenCalledWith("u1", "scheduled_task.toggle", expect.objectContaining({ taskId: "task1", status: "PAUSED" }));
    expect(mocks.auditUserAction).toHaveBeenCalledWith("u1", "scheduled_task.delete", expect.objectContaining({ taskId: "task1", name: "Clean logs" }), "WARNING");
