...
export function TicketDetailClient({ initial, canManage }: TicketDetailClientProps) {
  ...
  return (
    <div className="space-y-6">
      <Link href="/tickets" className="inline-flex items-center gap-1 text-xs text-slate-500 hover:text-slate-300 transition-colors">
        ← 返回工单列表
      </Link>
          ...
          <div className="flex flex-wrap gap-2">
            {TRANSITIONS[ticket.status].map((s) => (
              <button key={s} onClick={() => updateStatus(s)} disabled={saving}
                className="rounded-lg border border-white/10 bg-white/[0.04] px-4 py-2 text-sm text-white hover:bg-white/[0.08] transition-colors disabled:opacity-40">
      ...
      <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-5">
        ...
        <div className="mt-4">
          <textarea value={comment} onChange={(e) => setComment(e.target.value)} placeholder="添加评论..."
            rows={3}
            className="w-full rounded-lg border border-white/10 bg-white/[0.04] px-4 py-3 text-sm text-white outline-none placeholder:text-slate-600 resize-none" />
