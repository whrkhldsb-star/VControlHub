...
function getHealthPresentation(status: string | null | undefined, t: (k: string) => string) {
	switch (status) {
		...
		default:
			return { label: t("storagePage.list.unchecked"), className: "border-slate-400/30 bg-slate-400/10 text-slate-200" };
	}
}
...
function StorageNodeCard({
	...
	return (
		<article className="rounded-2xl border border-[var(--border)] bg-white/5">
					...
					{canManageNodes ? (
						<>
							<button
								...
								data-tone="cyan" className="inline-flex items-center justify-center w-11 h-11 rounded-lg border border-cyan-400/30 text-cyan-100 transition hover:bg-cyan-400/20"
							>
			...
			<p className="mt-2 text-xs text-slate-400">{t("storagePage.list.registeredFiles").replace("{count}", String(node.fileCount))}</p>
