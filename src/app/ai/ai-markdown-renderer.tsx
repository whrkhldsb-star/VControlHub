...
export const renderContent = (content: string) => {
 ...
 elements.push(
 <Tag key={`list-${elements.length}`} className={`ml-4 my-1.5 ${listType === "ol" ? "list-decimal" : "list-disc"} text-xs text-[var(--text-secondary)] space-y-0.5`}>
 ...
 <div className="flex items-center justify-between px-3 py-1.5 border-b border-[var(--border)]/5">
 <span className="text-[10px] text-cyan-400/60 font-mono">{lang || "code"}</span>
 <button
 onClick={() => copyToClipboard(code)}
 className="text-[10px] text-[var(--text-muted)] hover:text-cyan-300 transition opacity-0 group-hover/code:opacity-100 flex items-center gap-1"
 >
 ...
 4: "text-xs font-semibold", 5: "text-xs font-medium", 6: "text-xs font-medium text-[var(--text-muted)]",
 };
 elements.push(
 <div key={`h-${elements.length}`} className={`${sizes[level] || "text-xs"} text-[var(--text-primary)] mt-3 mb-1`}>
