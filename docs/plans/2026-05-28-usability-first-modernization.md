...
5. ✅ 2026-06-01 生产 QA 已完成 Direct Gateway 卡片恢复引导：展开 VPS 卡片时显示可探测公开 URL、当前直连/中转状态、监听端口/防火墙排查提示，以及切回网站中转会先尝试远端卸载再更新状态的真实副作用说明。
6. ✅ 2026-06-01 VPS 卡片状态与展开区可访问性已补齐：启用/停用徽章使用 `role="status"`，详情按钮绑定命名 region，展开态说明管理状态不等于 SSH/文件/直连健康，并改善浅色主题可读性。
7. 下一步继续验证编辑/删除安全路径和 SSH 终端空闲稳定性；删除/网关切换等高影响操作必须覆盖 DB、StorageNode、远端清理/卸载和下一屏可见结果。
