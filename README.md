...
- [ ] **后台任务业务迁移与并发控制仍在进行（P1）。** Durable Job 底座目前是第一阶段基础能力，尚未把所有长任务执行器迁入；后续需要为备份/恢复、SFTP 同步、QuickService、下载 direct/relay、定时任务/告警评估补实际 worker、全局/按用户/按节点并发上限、可观测日志流和生产级 worker 部署策略。
