...
- **文件状态一致性 / 远端索引刷新 / 存储列表性能（TR-005, P2）** — 删除/恢复主路径已避免高风险不一致；**远端 SFTP 索引定期校验（TR-005 T34a）已落地**：`storage.sftp-stale-inventory` durable job (30min interval) + 全树 SFTP 列表 + DB diff + soft-delete + POST `/api/storage/sftp-stale-inventory` 接口 (队列 202 + `?wait=1` 同步);WorkerId `sftp-stale-inventory` 加入 `lib/workers/registry.ts` 第 9 个 worker + 5min lease preset + `/api/admin/workers` 健康检查可见。**T34b.1 cursor 分页已落地**（commit `7d5a730`：`listFileEntries` / `listDeletedFileEntries` 加可选 `take/skip/cursor` 参数，默认行为不变 + 3 个新测）。**T34b.2 DB 聚合改造不可行**（queue T34b2 `failed_permanently`）：`previewable` 是运行时派生字段（`isPreviewableFile`），无法用 SQL where 替代内存 filter；`buildDirectorySummaries` 改成 `entryType: 'DIRECTORY'` 查询会破坏 P-001-T 行为兼容（`sftp-ops/route.ts:75` `upsertSftpFileIndex` 只建 FILE entry 不建 ancestor DIRECTORY entry，老数据 + 保存路径下 ancestor 不会注册）；caller `src/app/files/page.tsx:86` 仍要 `storage.entries` 拉全来 build file tree。后续 3 个重新评估选项：加 DB column `previewable` + 同步钩子 / 改 SFTP ops 写文件时同步建 ancestor DIRECTORY entry / 拆 `service-overview` 让 caller 改用 `listFileEntries`。
...
R1-R5 累计 17 路由, R6 (T38b) 走 storage 域 8 路由 (archive-download / direct-access / local / sftp / sftp-download / sftp-ops / sftp-stale-inventory / sftp-sync) 共用 `src/lib/storage/schema.ts` (10 个新共享 schema + 5 type export, 17 新 schema 单测), storage 域覆盖率 0% → 100%, 总体 13% → 47.8% (11/23 routes)。

R7 (T38c, commit `16126d9`) 走 ai 域 8 路由 (models / models.probe / conversations / conversations.[id] / chat / hosted-actions.[id] / providers / providers.[id]) 共用 `src/lib/ai/schema.ts` (9 个新共享 schema + 26 schema 单测)。**bug fix**: 原 inline `chatSchema` 要求 `message` 必填, 但 `ai-client.tsx` 实际发 `content` 字段, 等于每个 chat 请求 400; 新 schema 接受 `message` 或 `content` 任一字段。`scripts/tr-019-dto-audit.ts` 加 `extraBoundaryFiles` 支持多 boundary 识别 (dto.ts + schema.ts)。ai 域覆盖率 0% → 100%, 总体 11/16 → 19/24 (68.8% → 79.2%)。

R1-R7 累计 33 路由。R8 (T38d, commit `d15beca`) 走 files 域 3 路由 (archive-list / list / editable/[id]) 共用 `src/lib/files/schema.ts` (3 个新共享 schema + 15 schema 单测)。files 域覆盖率 0% → 100%, 总体 19/24 → 22/24 (79.2% → 91.7%)。R9 (T38e, commit `b7590cb`) 走 command 域 1 路由 (command-templates) + backup 域 1 路由 (backups/retention) 共用既有 `src/lib/command/schema.ts` + `src/lib/backup/schema.ts` (新增 3 schema + 32 schema 单测)。command 域 0% → 100%, backup 域 75% → 100%, 总体 22/24 → 24/24 (91.7% → 100%)。**TR-037 5 域全部 100% 覆盖, 收口。**
...
| TR-005 | P2 | 文件状态一致性、远端索引刷新、存储列表分页与内存聚合 | 远端索引 T34a 已落地 (durable job + 周期任务), T34b.1 cursor 分页已落地 (commit `7d5a730`), T34b.2 DB 聚合不可行 (派生字段限制) |
...
| TR-037 | P2 | API 入参 zod 校验补齐（39 个 route ad-hoc 解析） | ✅ R5 扫 2 路由 (operation-tasks + traffic/summary) + 修 helper bug。R1-R6 累计 25 路由 (storage 域 100%)。R7 (T38c, commit `16126d9`) 走 ai 域 8 路由 + 共用 `src/lib/ai/schema.ts` (9 schema + 26 单测) + 修 chatSchema bug (client 实际发 `content` 但 schema 要 `message` 每请求 400) + audit 脚本支持多 boundary。R8 (T38d, commit `d15beca`) 走 files 域 3 路由 + 共用 `src/lib/files/schema.ts` (3 schema + 15 单测)。R9 (T38e, commit `b7590cb`) 走 command 域 1 路由 + backup 域 1 路由 (扩既有 schema.ts, 0 新边界文件, +3 schema + 32 单测)。R1-R9 累计 38 路由, 总体 24/24 routes (100%), 5 域全部 100% 覆盖, 收口 |
...
- [x] **API 入参 zod 校验补齐**（TR-037）— 39 ad-hoc route 走 schema。R1-R5 累计 17 路由, R6 (T38b) 走 storage 域 8 路由 (共用 `src/lib/storage/schema.ts` 10 个共享 schema + 17 schema 单测 + 0→100% storage 域 + 修 sftp-ops 残 tsc `z.infer` namespace 错), R7 (T38c, commit `16126d9`) 走 ai 域 8 路由 (共用 `src/lib/ai/schema.ts` 9 schema + 26 单测) + 修 chatSchema bug (原要 `message` 必填, client 实际发 `content`, 100% 400 失败) + audit 脚本支持多 boundary, R8 (T38d, commit `d15beca`) 走 files 域 3 路由 (共用 `src/lib/files/schema.ts` 3 schema + 15 单测, 0→100% files 域), R9 (T38e, commit `b7590cb`) 走 command 域 1 路由 + backup 域 1 路由 (扩既有 `src/lib/command/schema.ts` + `src/lib/backup/schema.ts`, +3 schema + 32 单测, 0→100% command, 75%→100% backup), 总体 24/24 (100%)。**TR-037 5 域全部 100% 覆盖, 收口**。
