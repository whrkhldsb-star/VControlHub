...
- [ ] **前端可访问性、移动端和浏览器导航仍需系统化收口（P2）。** 全局搜索、Docker 日志、Docker 删除确认和 SSH 终端已具备统一 dialog focus 管理；文件浏览 SPA 已支持目录导航 pushState 与 popstate 恢复；SSH 终端仍有固定 `minHeight: 400px`、横向命令侧栏在手机上易拥挤的问题；文本预览搜索/跳转、SSH 常用命令输入等仍需继续从 placeholder-only 迁到可见 label。
