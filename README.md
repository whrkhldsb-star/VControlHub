...
- [ ] **后台任务业务迁移与并发控制仍在进行（P1）。** Durable Job 底座目前已承接 SFTP 同步、备份创建和备份恢复：显式写操作默认返回 `202 + jobId/taskId`，后台 worker 负责 lease/heartbeat/retry/结果记录，任务中心可展示进度和结果；`?wait=1` 仅作为旧同步响应兼容路径保留。后续仍需为 QuickService、下载 direct/relay、定时任务/告警评估补实际 worker、全局/按用户/按节点并发上限、可观测日志流和生产级 worker 部署策略。
...
- [ ] **前端可访问性、移动端和浏览器导航仍需系统化收口（P2）。** 全局搜索已具备 `dialog` 语义、初始聚焦、Escape 关闭和 opener focus restore；Docker 日志弹窗已接入统一 Dialog Focus hook；SSH 终端仍缺 Escape/focus trap/focus restore，且固定 `minHeight: 400px`、横向命令侧栏在手机上仍易拥挤；文本预览搜索/跳转、SSH 常用命令输入等仍依赖 placeholder 而缺显式 label；文件浏览使用 `replaceState` 更新目录 URL，浏览器后退不能逐级回到上一个目录。
...
- [ ] **文件预览/分享仍有部分入口未完全闭环（P1/P2）。** 公开目录分享已支持整体 tar.gz 下载，登录态 LOCAL/SFTP 受控下载已统一 Range/206 流层，媒体库 `/api/media/[id]/stream` 仍有一套局部 Range/header 实现，后续应迁入同一 helper；Office 预览和压缩包在线解压是安全降级入口，主要提供说明/下载，后续应在列表按钮和 README/API 文档里更早提示当前边界，或补受控格式解压。
...
- [ ] 设置页运行时反馈：展示当前运行值、配置来源、生效位置、是否需要重启，以及高风险设置的回滚/审计信息。
