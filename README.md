...
### P2 — 用户体验和可运营性
- [ ] 快捷服务生命周期：安装、启动/停止、状态刷新、更新和卸载（含可选数据目录清理）已闭环，生命周期审计历史已覆盖 started/succeeded/failed 事件，并已纳入统一 durable job worker；排队提示已能直达任务中心，继续补配置 diff、失败回滚、跨进程并发上限和更细日志流。
