						...
						<div key={t.id} className="px-5 py-4">
							<div className="flex items-center justify-between gap-3">
								...
								<div className="flex items-center gap-2">
									<span className="text-xs text-slate-500">{priorityLabels[t.priority] ?? t.priority}</span>
									<span className={`rounded-full border px-2.5 py-1 text-xs ${statusTone[t.status] ?? "border-white/[0.08] text-slate-400"}`}>
