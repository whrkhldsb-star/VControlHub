...
| 2 | **大文件拆分未完成 — 当前生产非测试非字典仍有 16 个 >500 行源文件**；本轮已拆分 `ai-ops-page-client.tsx`，主 orchestration shell 从 699 行降至 301 行，并提取 `ai-ops-sections.tsx` 作为纯展示组件 | 当前最大：`cost-page-client.tsx`(668)、`image-bed-page-client.tsx`(653)、`downloads-client.tsx`(645)、`quick-service/service-lifecycle.ts`(636)、`text-preview-client.tsx`(629)、`ssh-file-manager.tsx`(602) 等 | 继续按领域拆分：优先拆 UI 大组件（cost/image-bed/downloads/text-preview/quick-services）与 quick-service lifecycle |
| 3 | **大量 `.tsx` 组件仍含硬编码中文字符串** — 本轮已把主要真实 UI 文案批量迁移到 `t()`；当前生产非注释粗扫约 11 文件 / 20 行（剩余多为 provider 外 `global-error` 本地双语 copy、VPS backup preset 双语 label、压缩注释/安全注释等例外或低风险残留），`dashboard-localized-sections.tsx`、`settings/system-config-section.tsx`、`image-bed-page-client.tsx`、`files/*` 多个操作组件、`backups/*` 操作按钮、`operation-tasks/page.tsx`、`scheduled-tasks/page.tsx`、`quick-services/page.tsx`、`ai-ops-page-client.tsx` 已迁移到字典 | 当前剩余：`global-error.tsx`（Provider 外）、`vps-backup-section.tsx` preset label、少量压缩/注释误报和边缘组件；主要可见页面硬编码中文已大幅收敛 | 后续新增 JSX 文本/错误消息必须走 `t()` + 字典 key；`i18n:key-check` 仅校验已用 key，仍需定期扫描绕过 `t()` 的中文 |
