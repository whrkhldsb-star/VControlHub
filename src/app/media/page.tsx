      ...
      <section className="mb-5 grid gap-3 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="rounded-2xl border border-[var(--border)]/[0.07] bg-[var(--surface)]/[0.04] p-4">
          ...
          <div role="tablist" aria-label={t("mediaPage.aria.mediaType", locale)} className="mt-4 grid gap-2 text-sm sm:grid-cols-4">
            ...
            <FilterLink href={toggleTypeHref(filters, "audio")} active={mediaType === "audio"} activeClassName="border-emerald-400/55 bg-emerald-400/20 text-emerald-100" inactiveClassName="border-emerald-400/20 bg-emerald-400/[0.10] text-emerald-200 hover:bg-emerald-400/10" className="rounded-2xl border px-4 py-3 transition" title={mediaType === "audio" ? t("mediaPage.filter.titleToggleOffType", locale).replace("{type}", typeLabel(locale, "audio")) : t("mediaPage.filter.titleOnlyType", locale).replace("{type}", typeLabel(locale, "audio"))}>
