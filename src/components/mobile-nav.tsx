...
export function MobileNav({
	...
	return (
		<nav
			...
			data-i18n-skip
			className="fixed bottom-0 left-0 right-0 z-30 overflow-hidden border-t border-[var(--border)] bg-[color-mix(in_srgb,var(--modal-bg)_94%,transparent)] px-1.5 pb-[calc(0.4rem+env(safe-area-inset-bottom))] pt-1.5 shadow-[0_-8px_28px_color-mix(in_srgb,var(--text-primary)_8%,transparent)] backdrop-blur-xl light:bg-[color-mix(in_srgb,#ffffff_92%,var(--surface-subtle))] light:shadow-[0_-6px_20px_rgba(99,102,241,0.07)] md:hidden"
		>
			<div
				...
				{visibleTabs.map((tab) => {
					...
					return (
						...
						>
							...
							<span className="text-xs leading-tight max-[420px]:sr-only">{label}</span>
						</Link>
					);
				})}
