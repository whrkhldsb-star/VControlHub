...
| 7 | **input 字段缺 `<label>`/`aria-label`** — 已修复一批明确可交互输入；当前精扫剩约 25 处 / 17 文件（含 primitive 封装、自定义标签/checkbox、hidden/file 触发器过滤后的候选，仍需人工确认） | 剩余候选：`ui-primitives.tsx`、`input-base.tsx`、`share-file-picker.tsx`、`verify-2fa-form.tsx`、`media-item-card.tsx`、`image-bed-page-client.tsx`、`docker-resources-panel.tsx`、`ai-settings-panel.tsx`、`team-workspace-section.tsx`、`files/*` 等；已处理：`ssh-file-manager.tsx`、`sortable-step-card.tsx`、`step-config-editor.tsx`、`vps-backup-section.tsx` | 继续逐项人工确认；优先用可见 `<label htmlFor>`，确实无法显示 label 时才添加 `aria-label` |
