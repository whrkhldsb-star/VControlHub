...
R1-R7 累计 33 路由。R8 (T38d, commit `d15beca`) 走 files 域 3 路由 (archive-list / list / editable/[id]) 共用 `src/lib/files/schema.ts` (3 个新共享 schema + 15 schema 单测)。files 域覆盖率 0% → 100%, 总体 19/24 → 22/24 (79.2% → 91.7%)。**待续做**: command 域 1 路由 + 零散 1 路由 (T38e), 1 个 sub-tick 推到 ~95%。
