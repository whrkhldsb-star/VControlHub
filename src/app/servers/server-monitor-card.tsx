...
function usageTextColor(pct: number): string {
  if (pct >= 90) return "text-[var(--danger)]";
...
function ProgressBar({
  ...
  return (
    <div
      className={`h-1.5 rounded-full bg-[var(--surface)]/[0.10] overflow-hidden ${className}`}
    >
      {" "}
      <div
...
function MetricRow({
  ...
  return (
    <div className="flex items-center justify-between gap-2 text-xs">
      ...
      {pct !== undefined && (
        <ProgressBar value={pct} className="flex-1 min-w-[40px]" />
      )}{" "}
      <span
        className={`font-mono tabular-nums ${pct !== undefined ? usageTextColor(pct) : "text-[var(--text-primary)]"}`}
      >
        {" "}
        {value}
        {unit && (
...
export function ServerMonitorCard({ serverId }: Props) {
  ...
  const fetchMetrics = useCallback(async () => {
    try {
      const data = (await csrfFetch(
        `/api/servers/monitor?serverId=${encodeURIComponent(serverId)}`,
      )) as Metrics & { error?: string };
      if (data.error) {
        setError(data.error);
        setMetrics(null);
      } else {
      ...
      setError(
        err instanceof Error ? err.message : t("serverMonitor.error.fetch"),
      );
      setMetrics(null);
    } finally {
      setLoading(false);
  ...
  if (error) {
    return (
      <div
        data-tone="rose"
        className="rounded-xl border border-[var(--danger-border)] p-4"
      >
        {" "}
        <div className="flex items-center gap-2 text-xs text-[var(--danger)]">
          ...
          <span>⚠</span>{" "}
          <span>
            {t("serverMonitor.error.connection")}
            {t("common.colon")}
            {error}
          </span>{" "}
        </div>{" "}
        <button
          ...
          className="mt-2 text-xs text-[var(--danger)] hover:text-[var(--danger)] transition"
        >
          {" "}
          {t("common.retry")}{" "}
        </button>{" "}
  ...
  return (
    <div data-card className=" p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        {" "}
        <h4 className="text-xs font-medium text-[var(--text-primary)]/70 uppercase tracking-wider">
          {t("serverMonitor.title")}
        </h4>{" "}
      ...
      <div className="space-y-1.5">
        ...
        <div className="flex items-center justify-between text-[11px] text-[var(--text-muted)]">
          {" "}
          <span>{t("serverMonitor.cpu")}</span>{" "}
          <span className="text-[var(--text-muted)]">
            {cpu.cores} {t("serverMonitor.cores")}
          </span>{" "}
        </div>{" "}
        ...
        <div className="flex items-center justify-between">
          {" "}
          <span
            className={`text-sm font-semibold tabular-nums ${usageTextColor(cpu.usagePercent)}`}
          >
            {" "}
          ...
          <span className="text-[10px] text-[var(--text-muted)] font-mono">
            {" "}
            {t("serverMonitor.load")} {cpu.loadAvg[0].toFixed(2)} /{" "}
      ...
      <div className="space-y-1.5">
        ...
        <div className="flex items-center justify-between text-[11px] text-[var(--text-muted)]">
          {" "}
          <span>{t("serverMonitor.memory")}</span>{" "}
          <span className="text-[var(--text-muted)]">
        ...
        <span
          className={`text-sm font-semibold tabular-nums ${usageTextColor(memory.usagePercent)}`}
        >
          {" "}
