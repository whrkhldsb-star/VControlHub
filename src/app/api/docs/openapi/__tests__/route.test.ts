...
describe("GET /api/docs/openapi", () => {
  ...
  it("serves the authenticated OpenAPI spec at the route used by the API docs page", async () => {
    ...
    expect(Object.keys(body.paths)).toHaveLength(186);
    ...
    expect(body.paths["/files/preferences"]).toHaveProperty("patch");
    expect(body.paths["/storage/webdav-download"]).toHaveProperty("get");
