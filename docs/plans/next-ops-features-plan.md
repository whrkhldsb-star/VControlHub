...
5. **VPS/SSH 可用性 QA 方向**：SSH 终端基础可访问性/light-theme 修复已完成；Direct Gateway 卡片已补齐直连状态、公开 URL 探测、监听端口/防火墙恢复指导和切回中转副作用提示；VPS 卡片启用/停用徽章、详情区域和浅色主题状态说明也已补齐，并明确“已启用”只是 VControlHub 管理状态，不代表 SSH/文件/直连真实健康。下一步继续用生产浏览器验证 VPS 管理的真实用户路径：编辑/删除的安全边界、DB/StorageNode/Direct Gateway 清理副作用、终端空闲稳定性和复制/重试/恢复指导，确保“按钮可点”对应真实连接、配置或清晰失败状态。

## 架构收敛更新（2026-07-30）

第一批运维功能、权限、健康中心、统一任务、分享、备份和部署能力均已落地。后续不再以继续增加平行入口为主要方向，优先降低现有实现成本：

1. 前端请求统一走 typed API client；保留 `csrfFetch` 兼容调用，但不再维护第二套 CSRF、JSON 和错误处理实现。
2. 页面资源加载统一采用 `useResourcePolling` / `useVisibilityInterval`，避免重复初始化、重叠请求和后台标签页轮询。
3. UI primitive 只保留单一所有者；布局组件归 `page-shell`，表单/反馈控件归 `ui-primitives`。
4. 下一轮优先拆分 462 行 AI chat route，再评估 540 行 SFTP ops、494 行 server file proxy 和 488 行 thumbnail route；拆分必须同时带服务层测试，不能只移动代码。
