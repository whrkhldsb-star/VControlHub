...
| 3 | **大量 `.tsx` 组件仍含硬编码中文字符串** — 绕过 `t()` i18n 系统，EN locale 下直接显示中文；当前粗扫约 49 文件 / 153 行（含部分注释/常量误报），`audit/page.tsx` 与 `files/file-batch-toolbar.tsx` 已迁移到字典 | 当前热点：`requests/page.tsx`、`status/page.tsx`、`server-overview-card.tsx`、`requests/batch-review-toolbar.tsx`、`image-bed-page-client.tsx`、`files/move-inline-form.tsx`、`files/file-detail-panel.tsx`、`files/delete-confirm-button.tsx` 等 | 将所有 JSX 文本/错误消息迁移到 `t()` + 字典 key；`i18n:key-check` 脚本仅校验已用 key 的完整性，无法检测绕过 `t()` 的硬编码中文 |
