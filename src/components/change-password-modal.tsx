...
export function ChangePasswordModal({ open, onClose }: { open: boolean; onClose: () => void }) {
	...
	return (
		<div className="fixed inset-0 z-[100] flex items-center justify-center">
			...
			{/* Modal */}
			<div className="relative z-10 w-full max-w-md mx-4 rounded-3xl border border-white/10 bg-slate-900 p-6 shadow-2xl">
				...
				<form action={formAction} className="grid gap-4">
					<label className="grid gap-2 text-sm text-slate-300">
						...
						<input
							...
							className="rounded-2xl border border-white/10 bg-slate-950 px-4 py-3 text-white focus:outline-none focus:border-cyan-400/50"
							placeholder="请输入当前密码"
						/>
					</label>
					<label className="grid gap-2 text-sm text-slate-300">
					...
					<div className="flex justify-end gap-3 pt-2">
						<button
							type="button"
							onClick={onClose}
							className="rounded-2xl border border-white/10 px-5 py-2.5 text-sm text-slate-300 hover:bg-white/5 transition"
						>
							取消
						</button>
