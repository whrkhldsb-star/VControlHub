...
- **SSH 主机身份校验**：Server/StorageNode 增加 `hostKeySha256` 字段；SSH client 支持 `hostHash="sha256"` + `hostVerifier` pinning；存储 SSH 调用链透传 fingerprint。后续如果需要“首次连接 TOFU 审批 UI”，按产品功能单独推进。
