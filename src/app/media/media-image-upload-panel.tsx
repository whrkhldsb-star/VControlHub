	...
	return (
		<section className="mb-5 rounded-2xl border border-emerald-400/20 bg-emerald-400/[0.06] p-4 light:bg-emerald-50 light:border-emerald-200">
			<div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
				<div>
					<h2 className="text-base font-semibold text-emerald-100 light:text-emerald-900">图片图床工作区</h2>
					<p className="mt-1 text-xs text-emerald-100/70 light:text-emerald-800">在图片模式下统一上传、选择存储位置、生成外链；已有图片可在卡片上点“图床外链”。</p>
				</div>
				<div className="flex flex-wrap items-center gap-2 text-xs">
					<button type="button" onClick={loadNodes} disabled={loadingNodes} className="rounded-lg border border-emerald-300/30 px-3 py-2 text-emerald-100 transition hover:bg-emerald-400/10 disabled:opacity-60 light:text-emerald-800">
						{loadingNodes ? "加载节点中..." : nodesLoaded ? "刷新存储节点" : "加载存储节点"}
					</button>
					<button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading} className="rounded-lg bg-emerald-500 px-4 py-2 font-medium text-white transition hover:bg-emerald-400 disabled:opacity-60 light:text-emerald-950">
			...
			<div className="mt-3 grid gap-2 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
				<div className="text-xs text-emerald-100/80 light:text-emerald-800">
			...
			{progress ? (
				<div role="status" aria-label="媒体图片上传进度" className="mt-3 rounded-xl border border-emerald-300/20 bg-slate-950/40 p-3 text-xs text-emerald-100 light:bg-white/80 light:text-emerald-900">
			...
			{message ? <p role="status" className="mt-2 text-xs text-emerald-100 light:text-emerald-800">{message}</p> : null}
			{error ? <p role="alert" className="mt-2 text-xs text-rose-200 light:text-rose-700">{error}</p> : null}
		</section>
	);
}
