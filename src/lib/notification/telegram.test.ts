...
describe("telegram notification delivery", () => {
	...
	it("rejects disabled channel and missing bot token / chat ids", () => {
		...
		expect(() => assertTelegramReady({ enabled: false, botToken: "x", chatIds: ["1"] })).toThrow("Telegram 渠道未启用");
	...
	it("refuses to send when the Telegram channel is disabled", async () => {
		...
		await expect(sendAlertTelegram({ title: "x", message: "y" })).rejects.toThrow(/未启用|not enabled/);
