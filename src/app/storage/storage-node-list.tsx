...
export function StorageNodeList({
	...
	if (nodes.length === 0) {
		return <div className="rounded-2xl border border-dashed border-white/15 bg-white/5 p-5 text-sm text-slate-400 light:text-slate-600">暂无存储节点。</div>;
	}
...
function getHealthPresentation(status?: string | null) {
	switch (status) {
		...
		default:
			return { label: "未检测", className: "border-slate-400/30 bg-slate-400/10 text-slate-200 light:border-slate-300 light:bg-slate-100 light:text-slate-700" };
	}
}
...
function StorageNodeCard({
	...
	return (
		<article className="rounded-2xl border border-white/10 light:border-slate-200 bg-white/5 p-5">
			<div className="flex items-start justify-between gap-3">
				<div>
					<h3 className="text-lg font-medium text-white light:text-slate-900">{node.name}</h3>
					<p className="mt-2 text-sm leading-7 text-slate-300 light:text-slate-700">{node.connectionSummary}</p>
				</div>
				<div className="flex items-center gap-2">
					<span className="rounded-full border border-emerald-400/30 bg-emerald-400/10 px-3 py-1 text-xs text-emerald-100 light:text-emerald-900">
					...
					{canManageNodes ? (
						<>
							<button
								...
								title={editing ? "收起" : "编辑"}
								className="inline-flex items-center justify-center w-8 h-8 rounded-lg border border-cyan-400/30 bg-cyan-400/10 text-cyan-100 light:text-cyan-900 transition hover:bg-cyan-400/20"
							>
			...
			<p className="mt-3 text-sm text-cyan-100 light:text-cyan-900">
			...
			<div className="mt-4 rounded-2xl border border-white/10 light:border-slate-200 bg-slate-950/40 light:bg-white/40 p-4 text-sm text-slate-300 light:text-slate-700">
				<div className="flex flex-wrap items-center justify-between gap-3">
					<div className="flex flex-wrap items-center gap-2">
						<span className={`rounded-full border px-3 py-1 text-xs ${health.className}`}>{health.label}</span> <span>最近检测：{formatHealthTime(node.lastHealthCheckAt)}</span> {node.lastHealthLatencyMs != null ? <span>{node.lastHealthLatencyMs} ms</span> : null} </div> {canManageNodes ? ( <button type="button" onClick={handleHealthCheck} disabled={isPending} className="rounded-full border border-sky-400/30 bg-sky-400/10 px-3 py-1 text-xs font-medium text-sky-100 transition hover:bg-sky-400/20 disabled:cursor-not-allowed disabled:opacity-60" > {isPending ?"检测中..." :"立即检测"} </button> ) : null} </div> {node.lastHealthError ? <p className="mt-2 text-xs text-amber-200 light:text-amber-800">{node.lastHealthError}</p> : null} {message ? <p className={`mt-2 text-xs ${message.ok ? "text-emerald-200 light:text-emerald-800" : "text-rose-200 light:text-rose-800"}`}>{message.text}</p> : null}
			</div>
			<p className="mt-2 text-xs text-slate-400 light:text-slate-600">已登记文件：{node.fileCount}</p>
