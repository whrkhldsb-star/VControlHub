...
export function StorageNodeCreateForm({
  ...
  return (
    <form action={formAction} className="grid gap-4 rounded-3xl border border-[var(--border)] bg-slate-900/60 p-6">
      <div>
        <h2 className="text-xl font-semibold text-white">{t("storagePage.form.createTitle")}</h2>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <label className="grid gap-2 text-sm text-[var(--text-secondary)]">
          ...
          <input name="name" required className="rounded-2xl border border-[var(--border)] bg-slate-950 px-4 py-3 text-white" />
        </label>
        <label className="grid gap-2 text-sm text-[var(--text-secondary)]">
        ...
        <label className="grid gap-2 text-sm text-[var(--text-secondary)] md:col-span-2">
          ...
          <input name="basePath" required className="rounded-2xl border border-[var(--border)] bg-slate-950 px-4 py-3 text-white" placeholder={t("storagePage.form.basePathPlaceholder")} />
        </label>
        {isSftp ? (
          <>
            <label className="grid gap-2 text-sm text-[var(--text-secondary)]">
            ...
            <label className="grid gap-2 text-sm text-[var(--text-secondary)]">
              ...
              <input name="host" className="rounded-2xl border border-rose-400/40 bg-slate-950 px-4 py-3 text-white" placeholder={t("storagePage.form.hostPlaceholder")} />
            </label>
            <label className="grid gap-2 text-sm text-[var(--text-secondary)]">
              <span>{t("storagePage.form.fieldPort")}</span>
              <input name="port" type="number" min={1} max={65535} defaultValue={22} className="rounded-2xl border border-[var(--border)] bg-slate-950 px-4 py-3 text-white" />
            </label>
            <label className="grid gap-2 text-sm text-[var(--text-secondary)]">
              <span>{t("storagePage.form.fieldUsername")}</span>
              <input name="username" defaultValue="root" className="rounded-2xl border border-[var(--border)] bg-slate-950 px-4 py-3 text-white" />
            </label>
            <label className="grid gap-2 text-sm text-[var(--text-secondary)] md:col-span-2">
              <span>{t("storagePage.form.fieldAccessMode")}</span>
              <select name="directAccessMode" defaultValue="PROXY" className="rounded-2xl border border-[var(--border)] bg-slate-950 px-4 py-3 text-white">
            ...
            <label className="grid gap-2 text-sm text-[var(--text-secondary)]">
              ...
              <input name="publicBaseUrl" type="url" className="rounded-2xl border border-[var(--border)] bg-slate-950 px-4 py-3 text-white" placeholder={t("storagePage.form.publicBaseUrlPlaceholder")} />
            </label>
            <label className="grid gap-2 text-sm text-[var(--text-secondary)]">
              <span>{t("storagePage.form.fieldDirectExpiresSeconds")}</span>
              <input name="directAccessExpiresSeconds" type="number" min={60} max={86400} defaultValue={300} className="rounded-2xl border border-[var(--border)] bg-slate-950 px-4 py-3 text-white" />
            </label>
          </>
        ) : null}
