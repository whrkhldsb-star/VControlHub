...
- [x] **前端可访问性巡检可重复化（TR-003 收口，R19.C）。** 新增 `scripts/accessibility-audit.ts` + 20 个 `accessibility-audit.test.ts` 单元测试 + `npm run a11y:audit` 命令，对 `src/app/` + `src/components/` 222 个 `.tsx` 源文件做静态分析：每个 `<input>` / `<textarea>` / `<select>` 必须至少有 `aria-label` / `aria-labelledby` / `id` 配对 `<label htmlFor>` / 被 `<label>` 包裹 之一才算通过；结构性 input（`type="hidden|submit|button|reset|image"`）跳过；多行属性、JSX 表达式 `{...}`、模板字符串中的 `>`、自闭合 `<label/>` 全部由字符级 `findTagEnd` state machine 正确处理；输出 `docs/accessibility-audit.json`（235 字段 / 177 通过 / 58 跨 22 文件 待补全）便于按文件/行号定位。后续逐文件补全可访问 label 是 TR-003 下一轮切片，不属于本轮范围（脚本本身可重复运行）。 <!-- TR-003 -->
