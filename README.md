...
- [ ] **任务中心可观测性仍需继续治理（P1/P2）。** 任务中心已完成同类高频 completed 告警评估折叠，后续仍需补任务类型筛选/分组、失败/运行中优先视图、按来源聚合计数、失败原因聚合和更适合 cron 高频任务的保留/归档策略，避免长期历史数据继续拉低排查效率。
...
- [ ] **备份记录运维解释仍需继续治理（P2）。** 历史 PENDING/FAILED 记录已有显式作废入口；后续仍需补失败记录重试、失败原因归类、历史只读路径错误迁移说明、异地备份/自动恢复演练与保留策略自动清理，形成完整备份运维闭环。
