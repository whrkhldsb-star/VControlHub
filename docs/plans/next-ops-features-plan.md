...
5. **VPS/SSH 可用性 QA 方向**：SSH 终端基础可访问性/light-theme 修复已完成；Direct Gateway 卡片已补齐直连状态、公开 URL 探测、监听端口/防火墙恢复指导和切回中转副作用提示。下一步继续用生产浏览器验证 VPS 管理的真实用户路径：编辑/删除的安全边界、服务健康 badge、终端空闲稳定性和复制/重试/恢复指导，确保“按钮可点”对应真实连接、配置或清晰失败状态。
