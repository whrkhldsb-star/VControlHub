    ...
    try {
      const data = await csrfFetch<{ comment: TicketComment }>(`/api/tickets/${ticket.id}`, { method:"POST", headers: {"Content-Type":"application/json" }, body: JSON.stringify({ body: comment.trim() }), }); setTicket((prev) => ({ ...prev, comments: [...prev.comments, data.comment], updatedAt: new Date().toISOString() })); setComment(""); } catch (e: unknown) { setError(e instanceof Error ? e.message :"回复失败"); } finally { setSaving(false); } }; return ( <div className="space-y-6"> <Link href="/tickets" className="inline-flex items-center gap-1 text-xs text-slate-500 hover:text-slate-300 light:hover:text-slate-700 transition-colors"> ← 返回工单列表 </Link> {/* Ticket header */} <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-6"> <div className="flex items-start justify-between gap-4"> <div> <h1 className="text-xl font-semibold text-white light:text-slate-900">{ticket.title}</h1> <div className="mt-2 flex flex-wrap items-center gap-2"> <span className={`rounded-full border px-2.5 py-1 text-xs ${STATUS_TONE[ticket.status] ?? ""}`}>
        ...
        <div className="mt-4 rounded-lg border border-white/[0.04] bg-black/20 light:bg-slate-900/20 p-4">
          <p className="whitespace-pre-wrap text-sm text-slate-300 light:text-slate-700">{ticket.description}</p>
        </div>
         ...
         {canManage && (
           <div className="mt-3 flex items-center gap-2 text-xs">
             ...
             <select
               ...
               disabled={saving}
               className="rounded-lg border border-white/[0.08] bg-slate-950 light:bg-white px-2 py-1 text-sm text-slate-200 light:text-slate-800 outline-none disabled:opacity-50"
             >
      ...
      {canManage && TRANSITIONS[ticket.status]?.length > 0 && (
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-5">
          <h3 className="text-sm font-medium text-white light:text-slate-900 mb-3">状态流转</h3>
          <div className="flex flex-wrap gap-2">
            {TRANSITIONS[ticket.status].map((s) => (
              <button key={s} onClick={() => updateStatus(s)} disabled={saving}
                className="rounded-lg border border-white/10 light:border-slate-200 bg-white/[0.04] px-4 py-2 text-sm text-white light:text-slate-900 hover:bg-white/[0.08] transition-colors disabled:opacity-40">
      ...
      {/* Comments */}
      <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-5">
        <h3 className="text-sm font-medium text-white light:text-slate-900 mb-4">评论 ({ticket.comments.length})</h3>
            ...
            {ticket.comments.map((c) => (
              <div key={c.id} className="rounded-lg border border-white/[0.04] bg-black/20 light:bg-slate-900/20 p-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-white light:text-slate-900">{c.author.displayName || c.author.username}</span>
                  <span className="text-xs text-slate-500">{new Date(c.createdAt).toLocaleString("zh-CN")}</span>
                </div>
                <p className="whitespace-pre-wrap text-sm text-slate-300 light:text-slate-700">{c.body}</p>
              </div>
            ))}
          </div>
        )}
        ...
        <div className="mt-4">
          <textarea value={comment} onChange={(e) => setComment(e.target.value)} placeholder="添加评论..."
            rows={3}
            className="w-full rounded-lg border border-white/10 light:border-slate-200 bg-white/[0.04] px-4 py-3 text-sm text-white light:text-slate-900 outline-none placeholder:text-slate-600 light:placeholder:text-slate-500 resize-none" />
          <button onClick={addComment} disabled={saving || !comment.trim()}
            className="mt-2 rounded-lg bg-cyan-600 px-4 py-2 text-sm font-medium text-white light:text-slate-900 transition hover:bg-cyan-500 disabled:opacity-40">
            {saving ? "发送中…" : "发送评论"}
          </button>
        </div>
      </div>
